...
export const useGroupSettings = (groupid: string) => {
        ...
        mutationFn: async (values: z.infer<typeof GroupSettingsSchema>) => {
            if (values.thumbnail && values.thumbnail.length > 0) {
                ...
                const updated = await onUpDateGroupSettings(
                    ...
                    "IMAGE",
                    uploaded.uuid,
                    `/organizations/${groupid}/settings`,
                )
                if (updated.status !== 200) {
                    return toast("Error", {
                        description:
                            "Oops! looks like your form is empty or something broke",
                    })
                }
            }
    ...
    if (data?.status !== 200) router.push(`/organizations/create`)
...
export const useGroupInfo = () => {
    ...
    if (!data) router.push("/organizations")

    const { group, status } = data as { status: number; group: GroupStateProps }

    if (status !== 200) router.push("/organizations")
