...
const initialiseModel = () => {
  const model = new ChatGoogleGenerativeAI({
    modelName: "gemini-1.5-flash",  // Gemini's model name
    apiKey: process.env.GOOGLE_BOBB_API_KEY,
    temperature: 0.7,
  ...
  // Uncomment for OpenAI version later
  /*
...
function shouldContinue(state: typeof MessagesAnnotation.State) {
  ...
  if (lastMessage.content && lastMessage._getType() === "tool") {
    return "agent"; // Route back to agent if last message is a tool message
  }

  return END; // Otherwise, stop (reply to the user)
}
