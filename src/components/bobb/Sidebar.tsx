...
function ChatRow({
  ...
  const handleClick = () => {
    router.push(`/teacher/tools/assistants/bobb/${chat._id}`);
    closeMobileNav();
  };
...
export default function Sidebar({ clerkId }: Props) {
  const router = useRouter();
  ...
  const handleDeleteChat = async (id: Id<"chats">) => {
    await deleteChat({ id: id, clerkId: clerkId, });
    // If we're currently viewing this chat, redirect to dashboard
    if (window.location.pathname.includes(id)) {
      router.push("/teacher/tools/assistants");
    }
  };
